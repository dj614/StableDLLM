...
This repository is a small, opinionated scaffold for **masked-language diffusion** experiments, centered around the **LLaDA** family of diffusion language models.

It contains two main pieces:

- **`mdm/`**: a lightweight “framework layer” that provides
...
- **`LLaDA/`**: a vendored copy of the upstream LLaDA repo (tasks, metrics, original sampler, etc.). See `LLaDA/README.md` for upstream-specific notes.
...
- Python **3.10+**
- PyTorch, Transformers, Datasets, Accelerate, DeepSpeed (optional), WandB (optional)

A good baseline is the upstream dependency set plus `PyYAML` (used by the `mdm` config loader).
...
If you are pulling models/datasets from Hugging Face:
...
Training uses a **processed JSONL** format (see “Data format” below). The repo provides a few preprocessors under `src/tools/preprocess/train/`.
...
The repo includes a small convenience CLI at `src/llada/cli/main.py`:
...
There is an example Accelerate config at:
...
- `prompt_length`: the prefix length (tokens) that belong to the prompt. Loss is applied only to tokens **after** this boundary.
...
This format is produced by the preprocess scripts under `src/tools/preprocess/train/`.
...
Relevant config keys live under `train.*` (see `src/mdm/train/main.py` and `src/mdm/engines/llada_plus/cli/train.py`).
...
If your predictions do not already contain gold answers, you can also provide a ground-truth JSONL:
